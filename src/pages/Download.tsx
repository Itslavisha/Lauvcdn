...
export default function DownloadPage() {
  ...
  const getFileInfo = async () => {
    ...
    try {
      const actor = await getActor(storageIDL)
      const info = await actor.get_file_info(fileId)
      
      if (info.length > 0) {
        setFileInfo(info[0])
        setStatus('✅ File info retrieved successfully')
      } else {
      ...
      setStatus('❌ Error getting file info')
      setFileInfo(null)
    } finally {
      setIsLoading(false)
    }
  ...
  const downloadFile = async () => {
    ...
    try {
      ...
      const totalChunks = fileInfo.num_chunks
      const chunks = []
      
      // Download all chunks
      for (let i = 0; i < totalChunks; i++) {
        setStatus(`Downloading chunk ${i + 1}/${totalChunks}...`)
        const chunkResult = await actor.get_chunk(fileId, i)
  ...
  const previewChunk = async () => {
    if (!fileId) return
    
    setIsLoading(true)
    setStatus('Getting first chunk...')
    
    try {
      const actor = await getActor(storageIDL)
      const chunkResult = await actor.get_chunk(fileId, 0)
      
      if (chunkResult.length > 0) {
        const chunk = chunkResult[0]
        const text = new TextDecoder().decode(chunk.slice(0, 200))
        setStatus(`✅ First chunk preview (200 bytes): ${text}`)
      } else {
        setStatus('❌ No chunk found')
      }
    } catch (error) {
      console.error('Error getting chunk:', error)
      setStatus('❌ Error getting chunk')
    } finally {
      setIsLoading(false)
    }
  }
